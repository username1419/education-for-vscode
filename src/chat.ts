        ...
        switch (message.command) {
            case 'reqChatMessage': {
                ...
                if (!(typeof message.content.modelName === 'string')) {
                    logger.logError(`wrong message modelName: expected 'string' type instead of ${typeof message.model}`);
                    return;
                }
                if (!(typeof message.content.userPrompt === 'string')) {
                    logger.logError(`wrong message userPrompt: expected 'string' instead of ${typeof message.params}`);
                    return;
                }
